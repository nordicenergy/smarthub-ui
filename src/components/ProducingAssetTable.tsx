...
import { Certificate, CertificateLogic } from 'nordicenergy-snarthub-lib';
...
export class ProducingAssetTable extends PaginatedLoaderFiltered<ProducingAssetTableProps, IProducingAssetTableState> {
    ...
    async componentDidUpdate(nordicenergyProps: ProducingAssetTableProps) {
        if (nordicenergyProps.producingAssets !== this.props.producingAssets) {
    ...
    async enrichProducingAssetData(producingAssets: ProducingAsset.Entity[]): Promise<IEnrichedProducingAssetData[]> {
        const promises = producingAssets.map(
            async (asset) => ({
                ...
                organizationName: (await nordicenergy User(
                    asset.owner.address,
